...
int32 FAnimTurnInPlaceAnimSet::FindBestTurnTransitionIndex(float YawOffset) const
{
	...
	for (int32 Index = 0; Index < TurnTransitions.Num(); Index++)
	{
		if (const UAnimSequence* TurnTransitionAnim = TurnTransitions[Index].Anim)
		{
			...
			float PlayLength = TurnTransitionAnim->GetPlayLength();

			const FTransform TransitionAnimRootMotion = TurnTransitionAnim->ExtractRootMotionFromRange(0.f, /*TurnTransitionAnim->GetPlayLength()*/PlayLength);
...
void FAnimTurnInPlaceState::Update(float DeltaTime, bool bAllowTurnInPlace, bool bHoldYawOffset, bool bIsTurnTransitionStateRelevant, bool bClampYawOffset, float YawOffsetLimit,
	const FRotator& MeshWorldRotation, const FAnimTurnInPlaceAnimSet& AnimSet, UPARAM(ref) FAnimTurnInPlaceState& TurnInPlaceState, float TurnInPlaceSpeedMultiplier)
{
	...
	if (bAllowTurnInPlace)
	{
		...
		// alex add ability to clamp root yaw offset
		if (bClampYawOffset)
		{
			if (FMath::Abs(RootYawOffset) >= YawOffsetLimit)
				RootYawOffset = FMath::ClampAngle(RootYawOffset, -YawOffsetLimit, YawOffsetLimit);
		}
		
		// Update the current turn transition.
		if (bIsTurnTransitionStateRelevant)
		{
			UpdateActiveTurnTransition(DeltaTime, TurnInPlaceSpeedMultiplier);
		}
	...
	FString CurLogMessage = FString::Printf(TEXT("RootYawOffset(%.2f) PendingTurnAnim(%s) PendingTurnDelayCounter(%.2f) ActiveTurnAnim(%s) ActiveTurnAnimTime(%.2f) bTurnTransitionRequested(%d) bHoldYawOffset(%d) bIsTurnTransitionStateRelevant(%d)"), 
	...
	UE_LOG(LogAnimTurnInPlaceLibrary, VeryVerbose, TEXT("%s"), *CurLogMessage);
#endif // !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
}

void FAnimTurnInPlaceState::UpdateActiveTurnTransition(float DeltaTime, float TurnInPlaceSpeedMultiplier)
{
	#if WITH_EDITOR
	...
	const float NewTime = FMath::Min(ActiveTurnAnimTime + (DeltaTime * TurnInPlaceSpeedMultiplier), EndTime);
